...
proc_notif(){
    mpv $shutter_sound &
    ${notify_cmd_shot} -t 1500 "Screenshot" "Processing screenshot."
}
...
mv $tmp $filename
action=$(${notify_cmd_shot} "Screenshot" "Saved to ~/Pictures/Screenshots/${file}" --action " Open containing folder")
